						...
						<div class="tab-inner">
							<?php foreach ($quetions as $raw) { ?>
								<article class="question question-type-normal question_author_yes post-279 type-question status-publish hentry question-category-wordpress question_tags-php question_tags-wordpress" id="post-279" itemscope="" itemtype="http://schema.org/Article">
                                    ...
                                    <?php 
                                    $cond = array('quetions_id' => $raw['id']);
                                    $tbl = "reply";      
                                    $result = $this->HomeModel->select_cond_data($tbl, $cond); ?>
										<a href="<?= base_url() ?>/wp/knowledge/questions/php-recursive-function-not-working-right/#comments"><?=count($result)?> </a>
									</span>

									<span class="question-report question-statistic tooltip-s" data-original-title="Views">
										<span class="single-question-vote-result question_vote_result"><?=$raw['view']?></span>
									</span>
